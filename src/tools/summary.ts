...
export const summaryToolSchema = {
  name: 'ai_summary',
  description:
    '[DEMO] Summarize text with AI. Returns a concise summary with key points. ' +
...
export async function handleSummaryTool(
  ...
  const style = (args.style as string | undefined) ?? 'brief';
...
function generateMockedSummary(text: string, style: string, maxSentences: number): string {
  ...
  if (style === 'detailed') {
    ...
    if (selected.length === 0) {
      return (
        `The text provided contains ${wordCount} words and ${uniqueWords} unique terms. ` +
  ...
  return (
    `The provided text is ${wordCount} words long. ` +
